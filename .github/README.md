...
* **[The Symmetry Principle](../Documentation/symmetry_principle.md)**: Enforces a strict mapping where the directory name, CMake library target, translation unit source file, and namespaced library alias target are identical (`Directory Name == Target Name == Translation Unit File Name == Library Object Name`). This symmetrical architecture eliminates naming translation cognitive overhead, isolates compilation boundaries to prevent circular dependencies, and automates module renaming during refactoring.
* **[Dual-Preset Compilation Strategy](../Documentation/dual_preset_strategy.md)**: CMake configurations are separated into custom development presets (tailored for active local profiling with machine-specific optimization flags) and default distribution presets (ensuring zero-opinion portability for external users).
* **[Multi-Compiler Setup & Verification](../Documentation/multi_compiler_setup.md)**: Standardizing builds across GNU GCC, LLVM Clang, and Intel oneAPI compiler toolchains is a C++ software engineering best practice. Multi-compiler compilation guarantees strict ISO C++ standard compliance, prevents vendor-specific lock-in, and allows different optimizer backends to expose distinct memory bugs, performance bottlenecks, and compilation errors. To make this frictionless, the template orchestrates multiple compiler workflow presets sequentially under a unified verification script.
* **[Static Analysis & Clang-Tidy Workflow](../Documentation/static_analysis_workflows.md)**: The build system isolates static analysis diagnostics to maintain clean compilation logs. Structural and safety-critical findings (Category A) are checked directly in the active build pipeline. Non-critical situational design and stylistic findings (Categories B and C) are redirected automatically to the persistent state log (`clang_tidy_state.md`), providing a single-point summary containing clickable file links that navigate the IDE cursor directly to the target lines of code.
* **[Linux `perf` Profiling](../Documentation/profiling_guide.md)**: The build system exposes dedicated `Perf`, `Perf_multithread`, `Perf_record`, and `Perf_report` CMake targets for hardware-counter CLI analysis directly from the IDE target selector. These targets instrument the parallel workload against micro-architectural metrics — Instructions Per Cycle (IPC), L1/L2/L3 cache misses, branch misprediction rate, and thread migration counts — using the Linux `perf` subsystem without requiring any external tool installation. *Agent role: Performance Engineer.*
* **[Intel VTune & Advisor Integration](../Documentation/profiling_guide.md)**: The build system integrates Intel VTune and Intel Advisor as first-class profiling tools via dedicated CMake modules (`OneApi_Vtune.cmake`, `OneApi_Advisor.cmake`). VTune provides threading analysis (`Vtune_collect` / `Vtune_gui` targets) with a transient Systemd user service (`vtune-web.service`) as a web-based GUI backend to avoid Wayland/GTK crash issues. Advisor provides Roofline Model analysis (`Advisor_collect` / `Advisor_gui` targets) measuring arithmetic intensity (OP/Byte) and GFLOPS against hardware memory bandwidth ceilings. Both tools are activated through the `ENABLE_ITT` flag (automatically set in the `OneApi_Custom_RelWithDebInfo` preset), which compiles Intel ITT API instrumentation (`__itt_pause` / `__itt_resume`) directly into the binary to restrict data collection to the parallel hot-path and eliminate test harness noise. *Agent role: Performance Engineer.*
