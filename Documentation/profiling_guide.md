...
Inside the template's TBB parallel algorithm module (`srcTargets/tbbAlgos/tbbAlgos.cpp`), this is managed via preprocessor directives to ensure zero production overhead:
...
To leverage this low-noise instrumentation, start the VTune analysis with data collection paused:
...
> In modern Wayland Gnome environments, Intel VTune's CEF-based standalone desktop GUI often crashes due to GTK symbol version conflicts (GTK 2/3 vs 4). To solve this, the workflow relies on VTune's high-performance built-in web server backend (`vtune-backend`).

The project provides two custom CMake targets for VTune:
* **`Vtune_collect`**: Runs the VTune CLI collector for a **Threading Analysis** on the parallel database scanner, saving the results in `production_artifacts/profiling/vtune/`.
  
  > [!WARNING]
  > **Automatic Directory Clean & Overwrite Behavior:**
  > Every time the `Vtune_collect` target is executed, the script runs `rm -rf` on the prior results directory (`production_artifacts/profiling/vtune/`) before starting the trace. This prevents stale profiling data and lock conflicts, meaning **the folder always holds the absolute latest profiling run.**
...
Compiling or registering files on disk is not required. Start the dynamic service, check its status, or stop it using standard Systemd commands:
...
Once the service (or the `Vtune_gui` target) is running, connect to the VTune interface from a browser:
* **URL**: [https://localhost:8082/](https://localhost:8082/)
* **SSL Warning**: Because VTune uses a self-signed local SSL certificate, the browser will display a warning. It is safe to click **"Advanced"** and select **"Proceed to localhost (unsafe)"**.
* **Password**: `Lasse21man!` *(The local web server security passphrase)*

#### 3. Analyzing the Profiled Collection
* The server runs with the `--data-directory` pointing directly to the project's `/path/to/template2026/production_artifacts/profiling/` folder.
* Inside the Web GUI, the profile collection is visible in the right-hand panel under **"Recent Results"**.
* Since the `Vtune_collect` target automatically overwrites the `vtune` folder on every run, the result labeled **`vtune`** in the list will **always represent the latest profiling run**. Old stale data will never accumulate under that path.

#### 4. Running Collections while the Server is Active
Unlike the legacy standalone desktop GUI, **the VTune Web Server does not lock collection files during idle background execution.** 
* **Safe Concurrent Runs:** The `Vtune_collect` CMake target can be safely executed at any time while the background Systemd server is active. The directory cleanup (`rm -rf`) and new profiling run will succeed without any permission or access conflicts.
* **Viewing Updates:** Once the collection target completes in the terminal or CLion, the server automatically indexes the new data in the background. Refresh the browser page or click on the **"Recent Results"** panel again to view the updated profiling trace.
* **Browser Note:** If a detailed timeline graph is actively open in a browser tab at the exact moment the files are wiped by `Vtune_collect`, that specific tab may show a temporary loading error. Reload the page once the collection completes to view the new trace.
...
> 
> Because the custom CMake targets (`Advisor_collect` / `Vtune_collect`) perform a clean sweep and overwrite the target directories:
> 1. **Completely exit and close the standalone Advisor GUI before re-running the Advisor collection target.**
...
> 3. Once the collection target completes successfully in the console, the GUI can be safely re-launched via `Advisor_gui`.
> 
> *Note: This limitation **does not** apply to the new VTune Web Server workflow (`vtune-web.service` / `Vtune_gui` target). The web server can run continuously without locking conflicts.*
...
Compiling with the `OneApi_Custom_RelWithDebInfo` preset directs the Intel compiler (`icpx`) to generate a comprehensive optimization report detailing loop vectorizations, inlining decisions, and vectorization barriers.

* **Path:** **[compiler_opt_report.txt](../production_artifacts/profiling/advisor/compiler_opt_report.txt)**
* **Advisor GUI Integration:** By saving the optimization report directly inside the Advisor collection result directory, **Intel Advisor automatically finds and parses this report in the background.** This maps loop vectorization details, SIMD statistics, and inlining decisions directly to source code lines without requiring manual Search Path configuration.
