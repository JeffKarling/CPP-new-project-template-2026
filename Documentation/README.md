...
* **[Profiling & Performance Engineering](profiling_guide.md)**: Hardware-level performance analysis is critical, especially in autonomous or agent-assisted development workflows where AI-generated modifications must be evaluated for CPU-bound efficiency, thread load balancing, and cache utilization. Profiling provides empirical runtime metrics to ensure that code changes degrade neither scalability nor resource utilization, even when they pass functional unit tests. *Agent role: Performance Engineer.*
* **[Elephant/Goldfish Workflow Variant](agents/elephant_goldfish_workflow.md)**: The AI agent workspace implements a variant of the Elephant/Goldfish collaborative AI model. A persistent Elephant context (`GEMINI.md` + `agents.md`) retains full project knowledge across sessions, while five short-lived Goldfish roles execute narrowly scoped phases and hand off via a file-backed refactoring roadmap queue. New users familiar with the Elephant/Goldfish pattern should read this document first to orient themselves within the agent workspace.
