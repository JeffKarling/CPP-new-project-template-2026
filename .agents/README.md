...
> 6. **Git Branching and Versioning Policy**:
>    - AI agents MUST NEVER work directly on the `main` branch or perform push operations to GitHub unless explicitly directed by the user.
>    - **Default Mode**: If not specially instructed or operating in a Role-initiated session, the agent MUST conduct all development strictly on the branch named `next`. The `next` branch must always contain exactly one version bump at a given time (using agent discretion for patch, minor, or major bump). When adding changes to `next`, the agent MUST check if a version bump is already present; if not, the agent must bump the version.
>    - **Role-Initiated Mode**: Project special roles MUST always work on their own dedicated role branch where the branch name exactly matches the role name (e.g., `RefactoringSpecialist`).
>    - **Merging Strategy**: Agent role code changes on role branches are merged into `next` by a human. Subsequently, the `next` branch is merged into `main` by a human.
